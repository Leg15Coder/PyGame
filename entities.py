...
class Entity(Sprite):
    ...
    def set_img_for_player(self, dire: str, img: str):
        img = load_image(rf"sprites/{self.name}/{dire}/{img}.png", -1)
        self.sprite = pygame.transform.scale(img, (64, 64))
...
class Player(Entity):
    ...
    def walking(self):
        ...
        if self.events[115]:
            coords = (coords[0], coords[1] + self.speed)
            flag = True
            self.direction = 'behind_player'
        if self.events[100]:
            coords = (coords[0] + self.speed, coords[1])
            flag = True
            self.direction = 'right_player'
        if self.events[97]:
            coords = (coords[0] - self.speed, coords[1])
            flag = True
            self.direction = 'left_player'
        if self.check_move(coords):
            self.coords = coords
            self.set_pos(*self.coords)
    ...
    def update(self, event):
        ...
        self.walking()
        if self.state == 'stay':
            self.set_img_for_player(f"{self.direction}", f"{1}")
        elif self.state == 'walking':
            self.set_img_for_player(f"{self.direction}", f"{dt.now().microsecond // 1000 % 1000 // 334 + 1}")
        elif self.state == 'running':
            self.set_img_for_player(f"{self.direction}", f"{dt.now().microsecond // 500 % 500 // 167 + 1}")
...
class Questioners(Entity):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        img = load_image(rf"sprites/questioners/1.jpg", 0, True)
        self.sprite = pygame.transform.scale(img, (64, 64))
        self.behaviour = from_str_to_type(self.abilities['behaviour']) if 'behaviour' in self.abilities \
            else self.add_ability('behaviour', random)
        self.dialog = Dialog
